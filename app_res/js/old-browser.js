...
function PingPong(){
	...
	var topPositionCanvas = 0;

	window.addEventListener("mousemove", function(e) {
		let top = this.render.canvas.getBoundingClientRect().top;
	...
	function collisionCheck(player, ball){
		if(ball.x < player.x+config.player.width*config.scale && ball.x > player.x){
			...
			if(ball.y < player.y+config.player.height*config.scale && ball.y > player.y){
				//Cehck X punch
